...
class TwistNexusNode(Node):
    ...
    def loop(self):
        ...
        if publishers == 0:
            ...
            self.current_vx = self.ramp(self.current_vx, self.target_vx, self.linear_ramp_step)
            self.current_vy = self.ramp(self.current_vy, self.target_vy, self.linear_ramp_step)
