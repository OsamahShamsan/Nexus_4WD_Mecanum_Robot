...
class MoveRobotNode(Node):
    def __init__(self):
        ...
        self.f = 0.02
        if self.mode == 's':
            self.timer = self.create_timer(self.f, self.square_callback)
        elif self.mode == 'r':
            self.timer = self.create_timer(self.f, self.rotate_callback)
        elif self.mode == 'l':
            self.timer = self.create_timer(self.f, self.line_callback)
        else:
